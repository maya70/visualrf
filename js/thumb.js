	...
	$P.ThumbView = $P.defineClass(
			...
			},
			{
				drawThumbs: function(config){
					var self = this; 
					var treeFileLoader = config.treeFile || './php/load_pathway_trees.php';
					self.classes = {};
					self.genes = [];
					// Load tree quality measures for this pathway
					$P.getJSON('./php/load_tree_ROC.php', function(jsonData){
								self.treeQs = jsonData; 
						}, 
							{type: 'GET', 
							 data: {
							 	index: 0, 
					...
					$P.getJSON(treeFileLoader, function(jsonData){
							var tim = 1;
							var tids = [];
							var tempnodes = [];
							for(var i=0; i<jsonData.length; i++ ){
								var node = jsonData[i];
								var nodetree = node.treename;
								if(tim === nodetree ){
									//console.log("found a node of tree "+ti);
									tempnodes.push(node);
								}
								else{ //update tree id and process next tree
									tids.push(tim);
									// store the current tree's nodes
									self.fTrees[tim] = tempnodes;
									// clear the temporary treenodes
									tempnodes = [];
									// update to the new tree and its ID
									tim = nodetree; 
									tempnodes.push(node);
									//console.log("moving to next tree: "+ ti);
								}
							}
							// make sure to push the last tree
							if(tim === self.numTrees){
									tids.push(tim);
									// store the current tree's nodes
									self.fTrees[tim] = tempnodes;
							}
							...
							for(var ti=0; ti< 5; ti++){
								t = self.treeQs[ti].tid;
						
								var map = {}, node, roots = [];
								//console.log("tree: "+ t);
								// get the nodes of the current tree
								var nodes = self.fTrees[t];
	 							//self.updateControls(ti+1);		
	                            ...
	                            self.currentTree = roots[0];
	                            self.currentTree.treeQ = self.treeQs[ti];
	                            self.buildTreeRules(self.currentTree, '1');
	                            self.buildTreeInfo(self.currentTree, self.selectedPwayID, t, counter);
	                            self.buildTreeQuality(self.currentTree);
	                            //self.submitTree(self.currentTree, t, config);
	                            self.createColorMap();
	                            self.trees.push(self.currentTree);
	                            self.currentTree = [];
	                           counter++;

							}
							console.log(self.geneRelations);
							self.geneRelView = new $P.GeneRelView({'grel': self.geneRelations, 'colorMap': self.colorMap});
					...
					/*
					var t; 
					for(var ti =0; ti < self.numTrees ; ti++){
						t = self.treeQs[ti].tid; 
						console.log(t);
						var map = {}, node, roots = [];
						console.log("tree: "+ t);
						$P.getJSON(treeFileLoader,
	                        function(jsonData) {
	                            var nodes = jsonData;
	 							self.updateControls(ti+1);		
	                            for(var i=0; i < jsonData.length; i++)
	                            ...
	                            self.currentTree.treeQ = self.treeQs[ti];
	                            self.buildTreeRules(self.currentTree, '1');
	                            self.buildTreeInfo(self.currentTree, self.selectedPwayID, t);
	                            self.buildTreeQuality(self.currentTree);
	                            //self.submitTree(self.currentTree, t, config);
	                            self.createColorMap();
	                            self.trees.push(self.currentTree);
	                            self.currentTree = [];
	                            console.log("OVER HERE "+roots);
	                        },
	                        {
	                            type: 'GET',
	                            data: {
				   ...
				   //console.log("OVER THERE "+ roots);
				    self.drawIcicle(50, 50);
